...
namespace HostBridge
{
    partial class HostBridge : MhcsLib.SvcApp
    {
        ...
        protected override void AppStartup()
        {
            ...
            #endregion
            #if DEBUG
                SendAlarm("HostBridge", "Info", "Ripartenza del sistema NODE in modalità  DEBUG");
            #else
                SendAlarm("HostBridge", "Info", "Ripartenza del sistema NODE"); 
            #endif
        ...
        /// <summary>
        /// Lega gli eventi di dati pronti da leggere in OPC alle chiamate delle fuznioni
        ...
        {
            #region Parte dichiarativa
            string msg;         //Codice del comando Host da eseguire: RM,RS,MM,RI
            int ps;             //Progressivo di send
            int pa;             //Progressivo di ack
            int ra;             //Richiesta di ack
            ...
            {
                if (TokenLenght != 5)                                           //Si tratta di un messaggio di tipo "RM"
                {
                    InserRecordInHostLog("MhM ", "Msg",
                        tokens[0].PadLeft(5, '0') + "," + tokens[1].PadLeft(5, '0') + ","   + tokens[2] + "," + tokens[3] + "," +
                        tokens[4] + ","                 + tokens[5] + ","                   + tokens[6] + "," + tokens[7] + "," + 
                        tokens[8].Trim()  + ","         + tokens[9].PadLeft(5, '0'));
                ...
                {
                    InserRecordInHostLog("MhM ", "Msg",
                        tokens[0].PadLeft(5, '0') + "," + tokens[1].PadLeft(5, '0') + "," + tokens[2] + "," +
                ...
                case "RM":
                    {
                        ...
                        Priority = (Priority % 2 == 0) ? Priority = 0 : Priority = 1;             // PrioritÃ  indicata dal bit 0 della word id stato
                ...
                case "RI":
                    {
                        ...
                        sndmessage = PrepareToQueueSIMessage("PSAV01", "");         //Raccoglie tutti i dati necessari al messaggio per Host
                        QueueSMMsgToHost(sndmessage);                               //Salva il messaggio nel DB
                        sndmessage = PrepareToQueueSIMessage("PSAV02", "");
                        QueueSMMsgToHost(sndmessage);
                        sndmessage = PrepareToQueueSIMessage("PSAV03", "");
                        QueueSMMsgToHost(sndmessage);
                        sndmessage = PrepareToQueueSIMessage("PSAV04", "");
                        QueueSMMsgToHost(sndmessage);
                ...
                case "MM":
                    {
                        ...
                        try
                        {
                            ...
                            while (reader.Read())
                            {
                                if (!reader.IsDBNull(reader.GetOrdinal("TrOrderNr"))) Tid = reader.GetInt32(reader.GetOrdinal("TrOrderNr"));
                                ...
                                if (!reader.IsDBNull(reader.GetOrdinal("TrOrderState"))) Sts = reader.GetInt32(reader.GetOrdinal("TrOrderState"));
            ...
            else
            {
                ...
                switch (tokens[5])
                {
                    ...
                    case "1":   //Il trasporto è stato assegnato a un AGV
                        ...
                        break;
                    case "2":   //E' stato effettuato il prelievo
                        {
        ...
        public void ProcessLogMessage(string Message)
        {
            ...
            switch (Tokens[0])
            {
                case "AgvCtl":
                    {
                        DigAgvMessages = UnpackAgvCtlAlarm(Tokens[2]);
                        //AlarmText = UnpackDigAgvAlarm(Tokens[2]);
                        for (int i = 0; i < DigAgvMessages.Length; i++)
                        {
                            if (DigAgvMessages[i].Split(':').Length >= 2 &&
                                DigAgvMessages[i].Split(':')[1] != "")
                                InserRecordInAlarms(Tokens[0],
                                DigAgvMessages[i].Split(':')[0],
                                DigAgvMessages[i].Split(':')[1]);
                        }
                    }
                    break;
